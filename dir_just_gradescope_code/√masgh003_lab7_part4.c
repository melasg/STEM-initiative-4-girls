...
//unsigned short light_state; // already claimed in fc no?
unsigned short tmp, b = 0x00;
void ADC_init() {
    ADCSRA |= (1 << ADEN) | (1 << ADSC) | (1 << ADATE);
    ADMUX = 0x01;
}
void process(unsigned short light_state) {
	tmp = light_state/7;
    //The LEDs should illuminated in sequence from 0 to 7, based on the amount of light detected by the photoresistor.
    b = (tmp > 100) ? 0x01 : b;
}
...
int main(void) {
    ...
    while (1) {
        light_state = ADC;
        PORTB = process(light_state);
        PORTC = 0x08; //upperbound
    }
    return 1;
}
